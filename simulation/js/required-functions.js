...
export function makeGrid(x, y = -1) {
  ...
  grid.size = [x, y];
  grid.storedDirection = [];
  for (var i = 0; i < x; i++) {
    grid.storedDirection[i] = [];
    for (let j = 0; j < y; j++) {
      grid.storedDirection[i][j] = 0;
    }
  }
  return grid;
}
...
export const calculateNewValue = (x, y, reward=0, gamma=0.9, grid, returnValue = 1) => {
  ...
  return result[grid.storedDirection[x][y]-1];
};

export const completedOrNotForNow = (grid, newGrid) =>
{
  var diff = 0;
